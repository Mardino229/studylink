...
import { useMarkSolutionUnlocked } from '../../utils/exam';
...
export default function ExamSolution() {
    ...
    return (
        <div className="fixed inset-0 z-50 flex flex-col bg-white dark:bg-gray-900">
            <PageMeta title={title} description="Consulter le corrigé" />
            ...
            <div className="flex flex-1 flex-col overflow-hidden">
                ...
                {status === 'loading' && (
                    <div className="flex flex-1 flex-col items-center justify-center gap-4">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400">Chargement du corrigé…</p>
                    </div>
                )}
                ...
                {status === 'error402' && (
                    <div className="flex flex-1 flex-col items-center justify-center gap-6 px-6 text-center">
                        ...
                        >
                            <ArrowLeft size={12} className="inline-block" /> Retour à la bibliothèque
                        </button>
                    </div>
                )}
                ...
                {status === 'error404' && (
                    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Corrigé indisponible</h2>
                            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Aucun corrigé n'a encore été uploadé pour cette épreuve.
                            </p>
                        </div>
                        <button
                            onClick={() => navigate(-1)}
                ...
                {status === 'error' && (
                    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
                        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Une erreur est survenue</h2>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Impossible de charger le corrigé. Réessayez plus tard.
                        </p>
                        <button
                            onClick={() => navigate(-1)}
                            className="text-xs text-gray-400 underline underline-offset-2 hover:text-gray-600 dark:hover:text-gray-300"
