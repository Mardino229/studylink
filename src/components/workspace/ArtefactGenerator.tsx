...
const ArtefactGenerator: React.FC<ArtefactGeneratorProps> = ({ notebookId }) => {
    ...
    const handleGenerateSummary = async () => {
        try {
            ...
            const result = await generateSummaryMutation.mutateAsync({ notebookId, title: 'Résumé généré' });
    ...
    const handleGenerateQuiz = async () => {
        try {
            ...
            const result = await generateQuizMutation.mutateAsync({ notebookId, title: 'Quiz généré', count: 3 });
            setActiveArtefact(result);
        } catch (e) {
            console.error(e);
        }
    ...
    const handleGenerateFlashcards = async () => {
        try {
            ...
            const result = await generateFlashcardsMutation.mutateAsync({ notebookId, title: 'Flashcards générées', count: 5 });
            setActiveArtefact(result);
        } catch (e) {
            console.error(e);
        }
    ...
    const handleGeneratePodcast = async () => {
        try {
            ...
            const result = await generatePodcastMutation.mutateAsync({ notebookId, title: 'Podcast généré' });
            setActiveArtefact(result);
        } catch (e) {
            console.error(e);
        }
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
            ...
            {activeArtefact && artefactType === 'summary' && (
                <div className="bg-sky-50 border border-sky-100 p-4 rounded-lg mb-4">
                    <h4 className="font-semibold text-sky-800 mb-3">Résumé généré</h4>
            ...
            {activeArtefact && artefactType === 'quiz' && (
                <div className="bg-purple-50 border border-purple-100 p-4 rounded-lg">
                    <h4 className="font-semibold text-purple-800 mb-3">{activeArtefact.title || 'Quiz Généré'}</h4>
            ...
            {activeArtefact && artefactType === 'flashcards' && (
                <div className="bg-orange-50 border border-orange-100 p-4 rounded-lg">
                    <h4 className="font-semibold text-orange-800 mb-3">Flashcards Générées</h4>
            ...
            {activeArtefact && artefactType === 'podcast' && (
                <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-lg">
                    <h4 className="font-semibold text-emerald-800 mb-3">Podcast généré</h4>
            ...
            <div className="mt-6 space-y-4">
                <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Historique des résumés</p>
                    <div className="space-y-2">
                        {summaries?.items?.map(summary => (
                            <div key={summary.id} className="rounded-lg border border-gray-100 bg-gray-50 p-3 text-sm dark:border-gray-800 dark:bg-white/[0.03]">
                                <p className="font-medium text-gray-800 dark:text-white/90">{summary.title || 'Résumé'}</p>
                                <p className="mt-1 line-clamp-3 text-xs text-gray-500 dark:text-gray-400">{summary.content}</p>
                            </div>
                        ))}
                        {summaries?.items?.length === 0 && <p className="text-xs text-gray-400">Aucun résumé généré.</p>}
                    </div>
                </div>
                <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Historique des flashcards</p>
                    <div className="space-y-2">
                        {flashcards?.items?.map(batch => (
                            <div key={batch.id} className="rounded-lg border border-gray-100 bg-gray-50 p-3 text-sm dark:border-gray-800 dark:bg-white/[0.03]">
                                <p className="font-medium text-gray-800 dark:text-white/90">{batch.title || 'Flashcards'}</p>
                                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{batch.items.length} carte(s)</p>
                            </div>
                        ))}
                        {flashcards?.items?.length === 0 && <p className="text-xs text-gray-400">Aucune flashcard générée.</p>}
                    </div>
                </div>
                <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Historique des quiz</p>
                    <div className="space-y-2">
                        ...
                        {quizzes?.items?.length === 0 && <p className="text-xs text-gray-400">Aucun quiz généré.</p>}
                    </div>
                </div>
                <div>
                    <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Historique des podcasts</p>
                    <div className="space-y-2">
                        ...
                        {podcasts?.items?.length === 0 && <p className="text-xs text-gray-400">Aucun podcast généré.</p>}
                    </div>
                </div>
            </div>
        </div>
