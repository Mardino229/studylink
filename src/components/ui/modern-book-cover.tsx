...
const sizeMap = {
    sm: { width: "120px", spineTranslation: "102px" },
...
export const ModernBookCover = ({
    ...
    return (
        <div
            className={`z-10 cursor-pointer group [perspective:800px] w-min [--shadowColor:#bbb] dark:[--shadowColor:#111] ${className}`}
        >
            <div
                style={{
...
export const BookHeader = ({ children, className = "" }: BookHeaderProps) => (
    <div className={`flex gap-2 text-xs flex-wrap ${className}`}>{children}</div>
);
...
export const BookTitle = ({ children, className = "" }: BookTitleProps) => (
    <h1 className={`font-bold select-none mt-3 mb-1 text-xs text-balance ${className}`}>
        {children}
    </h1>
);
//
